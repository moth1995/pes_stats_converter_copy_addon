class PESMasterPlayer{
    ...
    GetBasicInfo(){
        this.name = this.doc.querySelector('.top-header span:not([class])').textContent;
        this.overall = parseInt(this.doc.querySelector('.top-header').querySelector("span").textContent);
        const rows = this.doc.querySelector('body > div.main-wrapper > div > div.main-stats-cards-container.container-large.flex.flex-expand > div.player-main-column.player-info-column > table').querySelectorAll("tr");        
        var info = {};
        rows.forEach(function(row){
            ...
            if (key=="Position")
                value = tds[1].querySelector("span").textContent.replace(/[\r\t\n]/gm, '').trim();
            info[key] = value;
        })
    ...
    StatTableToObject(table, dictionary){
        ...
        rows.forEach(function(row) {
            ...
            let dribbling = ["Ball Control", "Dribbling", "Tight Possession", "Balance"];
            let defending = ["Heading", "Jumping", "Defensive Awareness", "Tackling", "Defensive Engagement", "Aggression"];
            let passing = ["Low Pass", "Lofted Pass", "Set Piece Taking"];
            let physicality = ["Speed", "Acceleration", "Physical Contact", "Stamina"];
            let goalkeeping = ["GK Awareness", "GK Catching", "GK Parrying", "GK Reflexes", "GK Reach"];
    ...
    GetStats(){
        const statsTables = this.doc.querySelector('body > div.main-wrapper > div > div.main-stats-cards-container.container-large.flex.flex-expand > div.flex.flex-column > div.stats-container > div').querySelectorAll('table.player-stats-modern');
		this.stats = {};
		for(let index = 0; index < statsTables.length; index++){
			this.StatTableToObject(statsTables[index], this.stats);
		}
		const characteristicsTable = this.doc.querySelector('body > div.main-wrapper > div > div.main-stats-cards-container.container-large.flex.flex-expand > div.flex.flex-column > div.stats-container > div').querySelector('table.player-info');
		this.CharacteristicsTableToObject(characteristicsTable, this.stats);
		console.log(this.stats);
    }
	
	GetSpecialStats(){
		const lis = this.doc.querySelector('body > div.main-wrapper > div > div.main-stats-cards-container.container-large.flex.flex-expand > div.flex.flex-column > div.cards-container.flex.flex-expand').querySelectorAll('li');
		var specialStats = [];
	...
	GetPositions(){
		...
		rows.forEach(function(row){
			...
			positionsElements.forEach(function(position){
				let positionName = position.textContent.replace(/[\r\t\n]/gm, '').trim();
